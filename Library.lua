	...
	function Window:AddTab(Name: string, Icon)
		...
		function Tab:AddFullGroupbox(Name, IconName)
			...
			do
				...
				local BoxIcon = Library:GetIcon(IconName)
				if BoxIcon then
					New("ImageLabel", {
						Image = BoxIcon.Url,
						ImageColor3 = "AccentColor",
			...
			function Groupbox:Resize()
				...
				-- Make TabLeft full width and hide TabRight when this groupbox exists
				TabLeft.Size = UDim2.new(1, -6, TabLeft.Size.Y.Scale, TabLeft.Size.Y.Offset)
				TabRight.Visible = false
			end

			setmetatable(Groupbox, BaseGroupbox)
			Tab.HasFullWidth = true
		...
		local originalTabResize = Tab.Resize
		function Tab:Resize(ResizeWarningBox)

			if originalTabResize then
				originalTabResize(self, ResizeWarningBox)
			end

			if Tab.HasFullWidth then
				TabLeft.Size = UDim2.new(1, -6, TabLeft.Size.Y.Scale, TabLeft.Size.Y.Offset)
				TabRight.Visible = false
			end
		end
		...
		function Tab:Show()
			...
			if Tab.HasFullWidth then
				...
				TabLeft.Size = UDim2.new(
					...
					TabLeft.Size.Y.Scale,
					TabLeft.Size.Y.Offset
				)
		...
		function Tab:Hide()
			...
			-- Reset to normal layout when hiding
			TabLeft.Size = UDim2.new(
				0,
				math.floor(TabContainer.AbsoluteSize.X / 2) - 3,
				TabLeft.Size.Y.Scale,
		...
		function Tab:Resize(ResizeWarningBox)
			...
			-- Check if we have any full-width groupboxes
			local hasFullWidth = false
			for _, groupbox in pairs(Tab.Groupboxes) do
				if groupbox.IsFullWidth then
					hasFullWidth = true
					break
				end
			end
			...
			if hasFullWidth then
				-- Hide normal layout and show full width
